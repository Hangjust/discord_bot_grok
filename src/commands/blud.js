const bludWordReplacements = new Map([
  ['hello', 'whasgud'],
  ['hi', 'whasgud'],
  ['hey', 'ay'],
...
function translateToBludMode(text) {
  ...
  while (i < tokens.length) {
    ...
    if (/^[A-Za-z']+$/.test(token)) {
      ...
      if (bludWordReplacements.has(twoWord)) {
        replacement = bludWordReplacements.get(twoWord);
        // skip next token if we consumed it
        if (tokens[i+1] && /^\s+$/.test(tokens[i+1])) i++;
        if (tokens[i+1]) i++;
      } else if (bludWordReplacements.has(lower)) {
        replacement = bludWordReplacements.get(lower);
